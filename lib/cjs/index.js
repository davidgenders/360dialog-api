...
class WhatsAppAPI {
  ...
  constructor({
    ...
    if (v) this.v = v;
      ...
      console.warn(
        `[whatsapp-api-js] Cloud API version not defined. In production, it's strongly recommended pinning it to the desired version with the "v" argument. Defaulting to "${import_types2.DEFAULT_API_VERSION}".`
      );
      this.v = import_types2.DEFAULT_API_VERSION;
    }
    this.parsed = !!parsed;
  }
        ...
        new Promise((resolve) => {
          setTimeout(
            async () => {
              let phone;
              let message;
              if (message_builder instanceof import_types.ClientMessage) {
                phone = data;
  ...
  async verifyRequestSignature(raw_body, signature) {
    ...
    const data = encoder.encode((0, import_utils.escapeUnicode)(raw_body));
